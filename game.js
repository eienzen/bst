document.addEventListener("DOMContentLoaded", () => {
    ...
    let MAX_CONVERSION_LIMIT = 1000; // अधिकतम 1000 BST पॉइंट्स एक बार में कन्वर्ट हो सकते हैं
    let CONVERSION_RATIO = 1; // 1 BST पॉइंट = 1 BST टोकन (ओनर बदल सकता है)
    let isGameRunning = false;

    let playerData = JSON.parse(localStorage.getItem("playerData")) || {
        gamesPlayed: 0,
        totalPoints: 0, // BST पॉइंट्स
        totalRewards: 0, // BST टोकन
        boxesEaten: 0,
        pendingPoints: 0, // पेंडिंग BST पॉइंट्स
        totalReferrals: 0,
        referralPoints: 0, // रेफरल से BST पॉइंट्स
        pendingReferral: null,
        pendingReferrerPoints: 0,
        rewardHistory: [], // रिवॉर्ड हिस्ट्री (पॉइंट्स और टोकन)
        taskHistory: [], // टास्क हिस्ट्री
        stakingHistory: [], // स्टेकिंग हिस्ट्री
        ...
        lastLogin: 0 // डेली लॉगिन ट्रैक करने के लिए
    };
    ...
    const CONTRACT_ADDRESS = "0x3F1b918EF9c87b3Bb39D366FFD9F9Cb1375CD79B";
    ...
    let direction = "right";
    let boxesEaten = 0;
    let gamePoints = 0; // BST पॉइंट्स गेम से
    ...
    function draw() {
        ...
        document.getElementById("pendingPoints").textContent = `Pending Points: ${(playerData.pendingPoints || 0).toFixed(2)} BST Points`;
    }
    ...
    function move() {
        ...
        if (eatenBoxIndex !== -1) {
            eatingSound.play();
            boxesEaten++;
            const point = 0.5; // प्रत्येक बॉक्स के लिए 0.5 BST पॉइंट
            ...
            if (playerData.pendingReferral) {
                const referrerPoint = point * 0.01; // रेफरर को 1% पॉइंट
    ...
    function showGameOverPopup() {
        ...
        document.getElementById("finalPoints").textContent = `Earned Points: ${gamePoints.toFixed(2)} BST Points`;
    ...
    async function claimDailyLoginReward() {
        ...
        try {
            showLoading(true);
            const dailyReward = 1; // 1 BST पॉइंट डेली लॉगिन के लिए
    ...
    async function claimSocialMediaShareReward() {
        ...
        try {
            showLoading(true);
            const shareReward = 2; // 2 BST पॉइंट्स सोशल मीडिया शेयर के लिए
            ...
            alert("Failed to claim share reward: " + error.message);
        } finally {
            showLoading(false);
        }
    }
    ...
    async function convertPointsToTokens() {
        ...
        try {
            ...
            const pointsInWei = ethers.parseUnits(pointsToConvert.toString(), 18);
            const tokensToReceive = pointsToConvert / CONVERSION_RATIO;
            const tokensInWei = ethers.parseUnits(tokensToReceive.toString(), 18);
            const contractBal = await contract.contractBalance();
            if (ethers.toBigInt(contractBal) < tokensInWei) {
    ...
    async function stakeTokens() {
        ...
        try {
            ...
            if (ethers.toBigInt(walletBalance) < amountInWei) {
    ...
    async function connectWallet() {
        ...
        try {
            ...
            MAX_CONVERSION_LIMIT = Number(ethers.formatUnits(await contract.maxConversionLimit(), 18));
            CONVERSION_RATIO = Number(ethers.formatUnits(await contract.conversionRatio(), 18));
            await loadPlayerHistory();
            updatePlayerHistoryUI();
    ...
    async function loadPlayerHistory() {
        ...
        try {
            ...
            playerData.referralPoints = Number(ethers.formatUnits(history.referralRewards || 0, 18));
    ...
    function updatePlayerHistoryUI() {
        ...
        document.getElementById("totalGameRewards").textContent = `Total Token Rewards: ${(playerData.totalRewards || 0).toFixed(2)} BST Tokens`;
        ...
        document.getElementById("pendingPointsText").textContent = `Pending Points: ${(playerData.pendingPoints || 0).toFixed(2)} BST Points`;
        ...
        document.getElementById("lockedStakeBalance60D").textContent = `Locked Stake Balance (60D): ${(playerData.lockedStakeBalances[1] || 0).toFixed(2)} BST Tokens`;
        document.getElementById("lockedStakeBalance180D").textContent = `Locked Stake Balance (180D): ${(playerData.lockedStakeBalances[2] || 0).toFixed(2)} BST Tokens`;
        document.getElementById("lockedStakeBalance365D").textContent = `Locked Stake Balance (365D): ${(playerData.lockedStakeBalances[3] || 0).toFixed(2)} BST Tokens`;
        ...
        document.getElementById("taskHistoryList").innerHTML = (playerData.taskHistory || []).map(entry =>
            `<li>${entry.taskType}: ${entry.amount.toFixed(2)} BST Points on ${new Date(entry.timestamp).toLocaleString()}</li>`
        ).join("");
        document.getElementById("stakingHistoryList").innerHTML = (playerData.stakingHistory || []).map(entry =>
    ...
    document.getElementById("convertPoints").addEventListener("click", convertPointsToTokens);
